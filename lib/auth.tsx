...
/** Coerce PromiseLike -> Promise y aplica timeout para evitar “cargando infinito” */
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    const fetchOrCreate = useCallback(
        async (uid: string, mail: string | null) => {
            ...
            try {
                ...
                if (!mounted.current) return
                // Si no hay red, no bloquear la UI
    ...
    useEffect(() => {
        const init = async () => {
            ...
            setError(null)
            try {
                // getSession con timeout (evita spinner eterno si la red cuelga)
                const sess = await runWithTimeout(supabase.auth.getSession(), 4000)
                ...
                if (!mounted.current) return
                if (e?.message === 'TIMEOUT') setError('La sesión tardó demasiado. Reintenta.')
                else setError(e?.message || 'Error inicializando sesión')
            } finally {
        ...
        const { data: sub } = supabase.auth.onAuthStateChange(async (_evt, sess) => {
            ...
            try {
                ...
                if (mounted.current) setError(e?.message || 'No se pudo actualizar la sesión')
            } finally {
                if (mounted.current) setLoading(false)
            }
        })
